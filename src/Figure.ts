...
FigureSchema.statics.getUnparsed = async function(
  urls: string[]
): Promise<string[]> {
  const results: IFigure[] = await (<mongoose.Model<any>>(
    this.model(FigureString)
  ))
    .where("md5_url")
    .in(
      urls.map(url =>
...
FigureSchema.statics.getUnsaved = async function(
  figures: Array<IFigure>
): Promise<Array<IFigure>> {
  const results = await (<mongoose.Model<any>>this.model(FigureString))
    .where("md5_url")
    .in(figures.map(f => f.md5_url))
    .exec();
...
export const Figure: IFigureModel = mongoose.model<IFigure, IFigureModel>(
  FigureString,
  FigureSchema
);
