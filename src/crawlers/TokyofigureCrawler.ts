...
export class TokyofigureCrawler implements FigureCrawler {
  ...
  async fetchFigures() {
    ...
    const source = this.request.request(`${host}/products/list.php?category_id=9&disp_number=148&pageno=1wovn=ja`).pipe(
      ...
      map(([url, $]) => {
        ...
        const price = $('.sale_price_value').text().replace(/\n|\s/g, '');
        const publishAt = $('.sale_date > dd').text().replace(/\n|\s/g, '');
        const figure: Figure = {
          url,
          name,
          cover,
